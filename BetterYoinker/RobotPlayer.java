...
public strictfp class RobotPlayer {
    ...
    static boolean isYoinker = false;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        if (rc.getType() == RobotType.LANDSCAPER) {
            RobotInfo[] robots = rc.senseNearbyRobots();
            for (RobotInfo robot : robots) {
                if (robot.getType() == RobotType.HQ && robot.team != rc.getTeam() && rc.getLocation().distanceSquaredTo(robot.location) < 9) {
                    enemyHQ = robot.location;
                    rushScaper = true;
                    break;
                }
    ...
    static void runDesignSchool() throws GameActionException {
        ...
        for (RobotInfo robot : robots) {
            if (rc.getRoundNum() > 1300) {
                ...
                if (robot.getType() == RobotType.DELIVERY_DRONE && robot.team == rc.getTeam() && robot.currentlyHoldingUnit) {
                    landscaperCount++;
                }
            }
...
//        }
    ...
    static void runFulfillmentCenter() throws GameActionException {
        ...
        if (wallFilled() && rc.getRoundNum() < 1800) {
            if (tryBuild(RobotType.DELIVERY_DRONE, rc.getLocation().add(dirTo(hqLoc).rotateRight()))) {
                droneCount++;
            } else if (tryBuild(RobotType.DELIVERY_DRONE, rc.getLocation().add(dirTo(hqLoc).rotateLeft()))) {
                droneCount++;
    ...
    static void runLandscaper() throws GameActionException {
        if (rushScaper) {
            ...
            for (Direction dir : directions) {
                if (onWall(rc.getLocation().add(dir)) && sameAxis(dir)) {
                    if (trySenseElevation(rc.getLocation().add(dir)) < lowest) {
                        ...
                        lowest = trySenseElevation(rc.getLocation().add(dir));
                    }
                    if (frontTile == bigNum) {
            ...
            if (rc.getRoundNum() % 3 != 0 || Math.abs(trySenseElevation(rc.getLocation()) - lowest) > 3) {
...
//        }
    ...
    static void runDeliveryDrone() throws GameActionException {
        ...
        if (isChosenOne) {
            ...
            if (rc.getLocation().isAdjacentTo(hqLoc)) {
            ...
            } else if (rc.isCurrentlyHoldingUnit() && rc.getLocation().distanceSquaredTo(hqLoc) < 49) {
                for (Direction dir : directions) {
                    if (rc.canDropUnit(dir) && onWall(rc.getLocation().add(dir))) {
                        rc.dropUnit(dir);
                        break;
                ...
                if (enemyHQ == null) {
                    ...
                    if (rc.getLocation().distanceSquaredTo(targets[currentTarget]) < rc.getCurrentSensorRadiusSquared() - 10) {
                        currentTarget += 1;
                    }
                    RobotInfo[] robots = rc.senseNearbyRobots();
                    for (RobotInfo robot : robots) {
    ...
    //Just some redundant commented-out code
    /**
     * Team enemy = rc.getTeam().opponent(); MapLocation[] targets = new
     * MapLocation[]{new MapLocation(rc.getMapWidth() - hqLoc.x,
     * rc.getMapHeight() - hqLoc.y), new MapLocation(rc.getMapWidth() - hqLoc.x,
     * hqLoc.y), new MapLocation(hqLoc.x, rc.getMapHeight() - hqLoc.y)}; if
     * (isChosenOne && rc.getRoundNum() > 750) {
     * droneMoveTowards(targets[currentTarget]); if
     * (rc.getLocation().equals(targets[currentTarget])) { currentTarget += 1; }
     * RobotInfo[] robots = rc.senseNearbyRobots(); for (RobotInfo robot :
     * robots) { if (robot.getType() == RobotType.HQ && robot.team == enemy) {
     * if (trySendChain("420", robot.location.x, robot.location.y)) {
     * isChosenOne = false; }
     *
     * }
     * }
     * } else if (isChosenOne) { droneMoveTowards(mapMid); }
     * System.out.println(!isChosenOne && rc.getRoundNum() > 750 &&
     * enemyHQKnown); // wait till round 750 and until you know where enemy hq
     * is if (!isChosenOne && rc.getRoundNum() > 750 && enemyHQKnown) { // run
     * if you havent captured a landscaper yet if (!rc.isCurrentlyHoldingUnit())
     * { //scan for robots RobotInfo[] robots =
     * rc.senseNearbyRobots(rc.getCurrentSensorRadiusSquared(), enemy); //if any
     * robots were found and round is above 1000 if (robots.length > 0 &&
     * rc.getRoundNum() > 1000) { boolean noLandscapers = true; int closestID =
     * 0; int closestDistance = 1000; //run through all robots for (RobotInfo
     * robot : robots) { //only run if the robot found is a landscaper if
     * (robot.getType() == RobotType.LANDSCAPER && robot.team == enemy) {
     * noLandscapers = false; //if you can pick it up please do so if
     * (rc.canPickUpUnit(robot.getID())) { rc.pickUpUnit(robot.getID()); break;
     * } //find the closest landscaper if
     * (rc.getLocation().distanceSquaredTo(robot.location) < closestDistance) {
     * closestDistance = rc.getLocation().distanceSquaredTo(robot.location);
     * closestID = robot.ID;
     * }
     * }
     ...
     * if (robots.length > 0) { for (RobotInfo robot : robots) { if
     * (robot.getType() == RobotType.MINER || robot.getType() ==
     * RobotType.LANDSCAPER) { if (rc.canPickUpUnit(robot.getID())) {
     * rc.pickUpUnit(robot.getID()); break; } else {
     * droneMoveTowards(rc.senseRobot(robot.getID()).location); } } } } else {
     * if(notChosenCurrentTarget==0) { droneSwarmAround(mapMid);
     * if(rc.getLocation().isWithinDistanceSquared(mapMid,RobotType.NET_GUN.sensorRadiusSquared+5)){
     * notChosenCurrentTarget=1; } }if(notChosenCurrentTarget==1){
...
//            }
    ...
    static boolean droneSwarmAround(MapLocation loc, int padding) throws GameActionException {
        int radius = RobotType.NET_GUN.sensorRadiusSquared + padding;
        if (goRight) {
