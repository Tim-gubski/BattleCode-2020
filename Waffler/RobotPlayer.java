...
import static Waffler.RobotPlayer.robotBorder;
...
public strictfp class RobotPlayer {
    ...
    static MapLocation[] robotBorder = null;
        ...
        if (rushScaper) {
            ...
            for (Direction dir : directions) {
                ...
                if (rc.getLocation().add(dir).isAdjacentTo(hqLoc)) {
                    if (trySenseElevation(rc.getLocation().add(dir)) > highest && (rc.senseRobotAtLocation(rc.getLocation().add(dir)) == null || rc.senseRobotAtLocation(rc.getLocation().add(dir)).type == RobotType.DELIVERY_DRONE)) {
...
//            }
    ...
    static void setRobotBorder() {
        robotBorder = new MapLocation[]{
            new MapLocation(rc.getLocation().x, rc.getLocation().y + 1),
            new MapLocation(rc.getLocation().x + 1, rc.getLocation().y + 1),
            new MapLocation(rc.getLocation().x + 1, rc.getLocation().y),
            new MapLocation(rc.getLocation().x + 1, rc.getLocation().y - 1),
            new MapLocation(rc.getLocation().x, rc.getLocation().y - 1),
            new MapLocation(rc.getLocation().x - 1, rc.getLocation().y - 1),
            new MapLocation(rc.getLocation().x - 1, rc.getLocation().y),
    ...
    static void tryWaffle() throws GameActionException {
        for (MapLocation loc : robotBorder) {
            if (loc.x % 2 == 0 && loc.y % 2 == 0 && rc.getDirtCarrying() <= 0) {
                if (tryDigDirt(loc)) {
                    break;
                }
